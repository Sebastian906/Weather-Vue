            ...
            <div v-show="modalActive"
                ...
                <Transition name="modal-inner">
                    <div v-if="modalActive" class="p-4 bg-white self-start mt-32 max-w-screen-md">
                        <slot />
                        <button class="text-white mt-8 bg-weather-primary py-2 px-6" @click="$emit('close-modal')">Cerrar</button>
                    </div>
                </Transition>
            </div>
        </Transition>
...
.modal-inner-enter-active {
    transition: transform 0.3s cubic-bezier(0.52, 0.02, 0.19, 1.02) 0.15s;
}
